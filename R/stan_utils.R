...
vectorise_stan_lik = function(model_file, model_data, family = 'poisson',
  ...
  if(trend_model == 'RW'){
    if(any(grepl('// dynamic factor estimates', model_file, fixed = TRUE))){
      ...
      if(drift){
        init_trend_line <- grep('trend[1, s] ~ normal(0, sigma[1])',
                                model_file, fixed = TRUE) - 1
        model_file <- model_file[-c(init_trend_line:(init_trend_line + 2))]
        model_file[init_trend_line] <-
          'trend[1, 1:n_series] ~ normal(0, sigma);'

        remainder_line <- grep('trend[2:n, s] ~ normal(drift[s] + trend[1:(n - 1), s], sigma[1])',
                               model_file, fixed = TRUE) - 1
        model_file <- model_file[-c(remainder_line:(remainder_line + 2))]
        model_file[remainder_line] <-
          paste0('for(s in 1:n_series){\n',
                 'trend[2:n, s] ~ normal(drift[s] + trend[1:(n - 1), s], sigma[1]);\n',
                 '}')
        model_file = readLines(textConnection(model_file), n = -1)
      } else {
        init_trend_line <- grep('trend[1, s] ~ normal(0, sigma[1])',
        ...
        remainder_line <- grep('trend[2:n, s] ~ normal(trend[1:(n - 1), s], sigma[1])',
                               model_file, fixed = TRUE) - 1
        model_file <- model_file[-c(remainder_line:(remainder_line + 2))]
        model_file[remainder_line] <-
          paste0('for(s in 1:n_series){\n',
  ...
  if(trend_model == 'CAR1'){
    if(any(grepl('// dynamic factor estimates', model_file, fixed = TRUE))){
      ...
      if(drift){
        ...
        remainder_line <- grep('trend[2:n, s] ~ normal(drift[s] + ar1[s] * trend[1:(n - 1), s], sigma[1])',
                               model_file, fixed = TRUE) - 1
        model_file <- model_file[-c(remainder_line:(remainder_line + 2))]
        model_file[remainder_line] <-
          paste0('for(s in 1:n_series){\n',
                 'trend[2:n, s] ~ normal(drift[s] + pow(ar1[s], to_vector(time_dis[2:n, s])) .* trend[1:(n - 1), s], sigma[1]);\n',
                 '}')
        model_file = readLines(textConnection(model_file), n = -1)
      } else {
        init_trend_line <- grep('trend[1, s] ~ normal(0, sigma[1])',
        ...
        remainder_line <- grep('trend[2:n, s] ~ normal(ar1[s] * trend[1:(n - 1), s], sigma[1])',
                               model_file, fixed = TRUE) - 1
        model_file <- model_file[-c(remainder_line:(remainder_line + 2))]
        model_file[remainder_line] <-
          paste0('for(s in 1:n_series){\n',
                 'trend[2:n, s] ~ normal(pow(ar1[s], to_vector(time_dis[2:n, s])) .* trend[1:(n - 1), s], sigma[1]);\n',
                 '}')
        model_file = readLines(textConnection(model_file), n = -1)
      }
    }
  ...
  if(trend_model == 'AR1'){
    if(any(grepl('// dynamic factor estimates', model_file, fixed = TRUE))){
      ...
      if(drift){
        ...
        model_file[remainder_line] <-
          paste0('for(s in 1:n_series){\n',
                 'trend[2:n, s] ~ normal(drift[s] + ar1[s] * trend[1:(n - 1), s], sigma[1]);\n',
                 '}')
        model_file = readLines(textConnection(model_file), n = -1)
      } else {
        init_trend_line <- grep('trend[1, s] ~ normal(0, sigma[1])',
        ...
        model_file[remainder_line] <-
          paste0('for(s in 1:n_series){\n',
                 'trend[2:n, s] ~ normal(ar1[s] * trend[1:(n - 1), s], sigma[1]);\n',
                 '}')
        model_file = readLines(textConnection(model_file), n = -1)
      }
    }
  ...
  if(trend_model == 'AR2'){
    if(any(grepl('// dynamic factor estimates', model_file, fixed = TRUE))){
      ...
      if(drift){
        ...
        second_line <- grep('trend[2, s] ~ normal(drift[s] + trend[1, s] * ar1[s], sigma[1])',
                            model_file, fixed = TRUE) - 1
        model_file <- model_file[-c(second_line:(second_line + 2))]
        model_file[second_line] <-
        ...
        model_file[remainder_line] <-
          paste0('for(s in 1:n_series){\n',
                 'trend[3:n, s] ~ normal(drift[s] + ar1[s] * trend[2:(n - 1), s] + ar2[s] * trend[1:(n - 2), s], sigma[1]);\n',
                 '}')
        model_file = readLines(textConnection(model_file), n = -1)
      } else {
        init_trend_line <- grep('trend[1, s] ~ normal(0, sigma[1])',
        ...
        second_line <- grep('trend[2, s] ~ normal(trend[1, s] * ar1[s], sigma[1])',
                            model_file, fixed = TRUE) - 1
        model_file <- model_file[-c(second_line:(second_line + 2))]
        model_file[second_line] <-
        ...
        model_file[remainder_line] <-
          paste0('for(s in 1:n_series){\n',
                 'trend[3:n, s] ~ normal(ar1[s] * trend[2:(n - 1), s] + ar2[s] * trend[1:(n - 2), s], sigma[1]);\n',
                 '}')
        model_file = readLines(textConnection(model_file), n = -1)
      }
  ...
  if(trend_model == 'AR3'){
    if(any(grepl('// dynamic factor estimates', model_file, fixed = TRUE))){
      ...
      if(drift){
        ...
        model_file[remainder_line] <-
          paste0('for(s in 1:n_series){\n',
                 'trend[4:n, s] ~ normal(drift[s] + ar1[s] * trend[3:(n - 1), s] + ar2[s] * trend[2:(n - 2), s] + ar3[s] * trend[1:(n - 3), s], sigma[1]);\n',
                 '}')
        model_file = readLines(textConnection(model_file), n = -1)
      } else {
        init_trend_line <- grep('trend[1, s] ~ normal(0, sigma[1])',
        ...
        model_file[remainder_line] <-
          paste0('for(s in 1:n_series){\n',
                 'trend[4:n, s] ~ normal(ar1[s] * trend[3:(n - 1), s] + ar2[s] * trend[2:(n - 2), s] + ar3[s] * trend[1:(n - 3), s], sigma[1]);\n',
                 '}')
        model_file = readLines(textConnection(model_file), n = -1)
      }
  ...
  if(VAR1){
    ...
    model_file = readLines(textConnection(model_file), n = -1)
    model_file <- model_file[-c((grep("tau[s] = pow(sigma[1], -2.0);", model_file, fixed = TRUE) - 1):
                                  (grep("tau[s] = pow(sigma[1], -2.0);" , model_file, fixed = TRUE) + 1))]
